...
class HomeVC: UIViewController, Alertable {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if DataService.instance.currentUser != nil {
            DataService.instance.userIsDriver(userKey: (DataService.instance.currentUser?.uid)!, handler: { (status) in
                if status == true {
...
extension HomeVC: MKMapViewDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, didEnterRegion region: CLRegion) {
        DataService.instance.driverIsOnTrip(driverKey: (DataService.instance.currentUser?.uid)!, handler: { (isOnTrip, driverKey, passengerKey) in
            if isOnTrip == true {
